...
struct AtomicBorrowRef<'a> {
    borrow: &'a AtomicUsize,
}

impl<'a> AtomicBorrowRef<'a> {
    #[inline]
    fn new(borrow: &'a AtomicUsize) -> Self {
        let new = borrow.fetch_add(1, Ordering::Acquire) + 1;
        ...
        AtomicBorrowRef { borrow }
    }

    #[cold]
    #[inline(never)]
    fn do_panic(borrow: &'a AtomicUsize, new: usize) {
        if new == HIGH_BIT {
...
impl<'a> Drop for AtomicBorrowRef<'a> {
    #[inline]
    fn drop(&mut self) {
...
impl<'a> Clone for AtomicBorrowRef<'a> {
    #[inline]
    fn clone(&self) -> AtomicBorrowRef<'a> {
        AtomicBorrowRef::new(self.borrow)
    }
}

struct AtomicBorrowRefMut<'a> {
    borrow: &'a AtomicUsize,
}

impl<'a> Drop for AtomicBorrowRefMut<'a> {
    #[inline]
    fn drop(&mut self) {
...
impl<'a> AtomicBorrowRefMut<'a> {
    #[inline]
    fn new(borrow: &'a AtomicUsize) -> AtomicBorrowRefMut<'a> {
        ...
        AtomicBorrowRefMut { borrow }
    }
}

unsafe impl<T> Send for AtomicRefCell<T> where T: ?Sized + Send + Sync {}
unsafe impl<T> Sync for AtomicRefCell<T> where T: ?Sized + Send + Sync {}
