...
impl<'a> QueryGroupInfo<'a> {
    pub fn new(info: StorageGroupInfo<'a>) -> Self {
        Self {
    ...
    pub fn include(self, info: StorageGroupInfo<'a>) -> Option<Self> {
    ...
    pub fn exclude(self, info: StorageGroupInfo<'a>) -> Option<Self> {
        if self.group_family != info.group_family {
            return None;
        }
    ...
    pub fn group_range(self) -> Option<Range<usize>> {
        let group = unsafe { *self.group_family.as_ptr().add(self.group_offset as usize) };
        ...
        if self.query_mask == group.include_mask() {
            ...
            let prev_group = unsafe {
                *self
                    .group_family
                    .as_ptr()
                    .add((self.group_offset - 1) as usize)
            };
